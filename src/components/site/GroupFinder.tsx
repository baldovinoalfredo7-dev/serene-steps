...
function GroupCard({ g, variant }: { g: Group; variant: GroupFinderVariant }) {
  ...
  if (variant === "compact") {
    return (
      <Link
        to="/grupos/$slug"
        params={{ slug: g.slug }}
        className="flex h-full flex-col justify-between gap-6 rounded-2xl bg-paper p-7 shadow-soft ring-1 ring-black/5 transition-all hover:-translate-y-0.5 hover:shadow-lift"
      >
        <div>
  ...
  return (
    <article className="flex flex-col rounded-2xl bg-paper p-7 shadow-soft ring-1 ring-black/5 transition-all hover:-translate-y-0.5 hover:shadow-lift md:p-8">
