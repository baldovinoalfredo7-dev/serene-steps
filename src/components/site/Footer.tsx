import { Link } from "@tanstack/react-router";
import logoAsset from "@/assets/logo-aa.png.asset.json";

export function Footer() {
  return (
    <footer className="bg-brand-strong py-16 text-paper">
      <div className="mx-auto max-w-7xl px-6">
        <div className="flex flex-col justify-between gap-12 md:flex-row">
          <div className="max-w-[42ch]">
            <div className="mb-5 flex items-center gap-3">
              <img
                src={logoAsset.url}
                alt=""
                width={44}
                height={44}
                className="h-11 w-auto rounded-lg bg-paper p-1"
                aria-hidden
              />
              <div className="leading-tight">
                <span className="block text-sm font-bold uppercase tracking-[0.14em] text-paper">
                  Alcohólicos Anónimos
                </span>
                <span className="block text-xs font-medium text-paper/75">
                  Área 2 Metropolitana – Barranquilla
                </span>
              </div>
            </div>
            <p className="text-sm leading-relaxed text-paper/80">
              Comunidad de hombres y mujeres que comparten su experiencia,
              fortaleza y esperanza para resolver su problema común y ayudar a
              otros a recuperarse del alcoholismo.
            </p>
          </div>

          <div className="grid gap-10 sm:grid-cols-4 sm:gap-14">
            <FooterColumn
              title="Ayuda"
              links={[
                { to: "/necesito-ayuda", label: "Busco ayuda" },
                { to: "/grupos", label: "Encuentra un grupo" },
                { to: "/primera-reunion", label: "Primera reunión" },
              ]}
            />
            <FooterColumn
              title="Comunidad"
              links={[
                { to: "/que-es-aa", label: "Bienvenido a tu casa en el Área 2" },
                { to: "/caribe", label: "AA en el Caribe colombiano" },
                { to: "/literatura", label: "Nuestra literatura" },
              ]}
            />
            <FooterColumn
              title="Cooperación"
              links={[
                { to: "/contacto", label: "Trabajemos juntos" },
                { to: "/contacto", label: "Contacto" },
              ]}
            />
            <FooterColumn
              title="Miembros"
              links={[{ to: "/auth", label: "Acceso para miembros" }]}
            />
          </div>
        </div>

        <div className="mt-16 flex flex-col items-start justify-between gap-3 border-t border-paper/20 pt-8 text-[11px] font-medium uppercase tracking-widest text-paper/65 md:flex-row md:items-center">
          <span>© {new Date().getFullYear()} AA Área 2 Metropolitana · Barranquilla, Colombia</span>
          <span className="italic normal-case tracking-normal text-paper/75">
            El anonimato es la base espiritual de nuestras tradiciones
          </span>
        </div>
      </div>
    </footer>
  );
}

function FooterColumn({
  title,
  links,
}: {
  title: string;
  links: { to: string; label: string }[];
}) {
  return (
    <div className="space-y-4">
      <h5 className="text-xs font-bold uppercase tracking-widest text-paper">{title}</h5>
      <ul className="space-y-2.5 text-sm text-paper/80">
        {links.map((l) => (
          <li key={`${l.to}-${l.label}`}>
            <Link to={l.to} className="transition-colors hover:text-paper">
              {l.label}
            </Link>
          </li>
        ))}
      </ul>
    </div>
  );
}
